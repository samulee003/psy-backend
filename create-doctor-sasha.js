...
console.log('=== 創建 [REDACTED]@gmail.com 醫生帳號 ===\n');
...
async function createDoctor() {
  try {
    ...
    // 檢查是否已存在
    db.get('SELECT * FROM users WHERE email = ?', ['[REDACTED]@gmail.com'], (err, existingUser) => {
      if (err) {
      ...
      db.run(query, [
        ...
        if (err) {
          ...
          console.log(`  - 郵箱: [REDACTED]@gmail.com`);
          console.log(`  - 密碼: [REDACTED]`);
